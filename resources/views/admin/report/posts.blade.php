...
<div class="report-page">
    <div class="report-shell">
        <div class="report-header">
            <div>
                <h1>Laporan Postingan</h1>
                <p>Daftar laporan dari user untuk konten postingan.</p>
            </div>
        </div>
        ...
        <div class="report-stats">
            <div class="report-stat-card">
                ...
                <span>Total laporan postingan yang sedang ditampilkan.</span>
            </div>
            <div class="report-stat-card">
            ...
            <div class="report-stat-card">
                <strong>{{ number_format($reports->filter(fn ($report) => optional($report->post)->status === 'banned')->count()) }}</strong>
                <span>Posting pada daftar ini yang sudah dibanned.</span>
            </div>
        </div>

        <div class="report-card">
            <form action="{{ route('admin.report.post') }}" method="GET">
                <div class="row g-3">
                    <div class="col-md-9">
            ...
            <div class="table-wrap">
                <table class="report-table">
                    ...
                    <tbody>
                        @forelse ($reports as $report)
                            <tr>
                                ...
                                <td>
                                    ...
                                    @else
                                        <span class="muted-text">Postingan sudah dihapus.</span>
                                    @endif
                                </td>
                                ...
                                <td>
                                    <div class="action-row">
                                        ...
                                        @if($report->post && $report->post->status !== 'banned')
                                    ...
                                    <div class="modal-content border-0 rounded-4">
                                        ...
                                        <div class="modal-body pt-2">
                                            <div class="row g-4">
                                                ...
                                                @if($report->post)
                                                    <div class="col-12">
                                                        <h6>Caption Postingan</h6>
                                                        <p class="mb-0">{{ $report->post->caption ?: 'Tanpa caption' }}</p>
                                                        @if(optional($report->post->photos->first())->photo)
                                                            <img src="{{ asset('storage/' . $report->post->photos->first()->photo) }}" class="modal-preview" alt="Preview post">
                                                        @endif
                                                    </div>
                                                @endif
                                            </div>
                            ...
                            @if($report->post && $report->post->status !== 'banned')
                                <div class="modal fade" id="banModal{{ $report->id }}" tabindex="-1" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content border-0 rounded-4">
                                            <form action="{{ route('admin.post.ban', $report->post->id) }}" method="POST">
                                                @csrf
                                                @method('PATCH')
                                                <div class="modal-header border-0">
                                                    <h5 class="modal-title text-danger">Ban Postingan</h5>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                                </div>
                                                <div class="modal-body pt-0">
                                                    <p class="text-muted">Postingan dari <strong>{{ $report->reportedUser?->name }}</strong> akan diblokir dari dashboard user.</p>
                                                    <div class="mb-3">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="empty-state">Belum ada laporan postingan yang cocok dengan filter saat ini.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
